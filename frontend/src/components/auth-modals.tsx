...
// Extended signup schema with confirm password
...
type SignUpWithConfirmForm = z.infer<typeof signupWithConfirmSchema>;
...
export default function AuthModal({ isOpen, onClose, mode, setMode }: AuthModalProps) {
	...
	const signupForm = useForm<SignUpWithConfirmForm>({
					...
					{mode === "login" && (
						<form onSubmit={loginForm.handleSubmit(handleLoginSubmit)} className="space-y-4">
							...
							{Object.keys(loginForm.formState.errors).length > 0 && (
								<div className="mb-4 rounded-md border border-red-200 bg-red-50 p-3" role="alert">
									<div className="text-sm text-red-600">
										{Object.entries(loginForm.formState.errors).map(([field, error]) => (
							...
							<button
								type="submit"
								disabled={isPending}
								className="w-full rounded-md bg-blue-600 px-4 py-2 text-white transition-all duration-200 hover:bg-blue-700 hover:shadow-lg focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
							>
					...
					{mode === "register" && (
						<form onSubmit={signupForm.handleSubmit(handleSignupSubmit)} className="space-y-4">
							...
							{Object.keys(signupForm.formState.errors).length > 0 && (
								<div className="mb-4 rounded-md border border-red-200 bg-red-50 p-3" role="alert">
									<div className="text-sm text-red-600">
										{Object.entries(signupForm.formState.errors).map(([field, error]) => (
											<div key={field}>{error?.message}</div>
										))}
									</div>
								</div>
